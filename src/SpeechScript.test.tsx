...
import { SpeechScriptJSX } from './SpeechScript';
...
class ScriptWithOptions extends SpeechScriptJSXWithOption<{
  ...
  speech() {
    const { username, launchCount } = this.options;
    return (
      <speak>
        <p>
          Hello {username}-san. You launch it by {launchCount} times. How are
          you?
        </p>
      </speak>
    );
...
describe('SpeechScriptJSXWithOption', () => {
  ...
  it('should include the option', () => {
    expect(speechScript.createResponse()).toEqual({
      outputSpeech: {
        ssml:
          '<speak><p>Hello John-san. You launch it by 5 times. How are you?</p></speak>',
        type: 'SSML',
      },
    });
  });
...
describe('SpeechScript', () => {
  ...
  describe('create', () => {
    ...
    it('should return stringify speech ssml', () => {
      expect(response).toMatchObject({
        speech:
          "<speak><p>Hello! It's a nice development. How are you?</p></speak>",
      });
    });
  ...
  describe('createResponseBuilder()', () => {
    ...
    const responseBuilder = new LaunchRequestScript(
      handlerInput
